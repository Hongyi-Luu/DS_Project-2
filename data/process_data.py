...
def load_data(messages_filepath, categories_filepath):
    ...
    Returns:
    df (DataFrame): Merged dataframe containing messages and their corresponding categories.
    """
    # Load datasets
...
def clean_data(df):
    """
    Clean the combined dataframe by splitting categories and converting category values to binary.
    
    Args:
    df (DataFrame): Combined dataframe with messages and categories.
    ...
    category_colnames = row.apply(lambda x: x[:-2]) # extract new column names for categories
...
def main(messages_filepath=None, categories_filepath=None, database_filepath=None):
    if messages_filepath and categories_filepath and database_filepath:
        print('Loading data...\n    MESSAGES: {}\n    CATEGORIES: {}'
              .format(messages_filepath, categories_filepath))
        df = load_data(messages_filepath, categories_filepath)
